...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < x; i++)
    {
        ...
        returnStatus = read(simpleSocket, &resp, sizeof(int));

        if (returnStatus > 0)
        {
            printf("%d: risposta %d - %d\n", returnStatus, i, resp);
        }
        else
        {
            fprintf(stderr, "Return Status = %d \n", returnStatus);
    ...
    double serviceTime;
        ...
        returnStatus = read(simpleSocket, &serviceTime, sizeof(double));

        if (returnStatus > 0)
        {
            printf("Tempo impiegato: %.2lf secondi\n", serviceTime);
        }
        else
        {
            fprintf(stderr, "Return Status = %d \n", returnStatus);
