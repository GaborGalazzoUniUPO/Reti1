...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        snprintf(dynamic_message, sizeof(dynamic_message), "Current Server Timestamp: %.24s\r\n", ctime(&ticks));


        /* handle the new connection request  */
        /* write out our message to the client */
        int e = write(simpleChildSocket, dynamic_message, strlen(dynamic_message));
        close(simpleChildSocket);
        if(e != strlen(dynamic_message)){
